...
export const resetPassword = async (email, old_password, new_password) => {
    const user = await getUser(email);
    if (!user)
        throw new Error("User not found");
    const hashedOldPassword = createHash("SHA256")
        ...
        .digest("base64");
    try {
        !timingSafeEqual(Buffer.from(hashedOldPassword), Buffer.from(user.password));
    }
    catch (error) {
...
export const getReports = async (query, orderBy) => {
    ...
    if (orderBy)
        reports = reports.sort(sortBy(orderBy === "penalty" ? "-penalty_count" : orderBy));
    return reports;
};
export const getReport = async (driver_licence) => {
    const { rows } = await pool.query("SELECT * FROM ietp_reports WHERE driver_licence=$1", [driver_licence]);
    return rows[0];
};
export const createReport = async (newReport) => {
    const { driver_name, driver_licence, car_plate, speed, report_date, city } = newReport;
    ...
    await pool.query("INSERT INTO ietp_reports (driver_name, driver_licence, car_plate, speed, penalty_count, report_date, city) VALUES ($1, $2, $3, $4, $5, $6, $7)", [driver_name, driver_licence, car_plate, speed, 1, report_date, city]);
